...
Or if you're able to work with "github's" mechanics:
...
3. Build the translation with `make i18n-build`
4. Test the translation with `make dev COUNTRY=country_CODE` (or `./scripts/test.sh country_CODE`)

   **Note** : You need install language pack first.

## Scripts

- `make i18-merge` (or `./scripts/translate-merge.sh`) will parse the `i18n()` calls in the `*.qml` files and write it to the
  `template.pot` file. Then it will merge any changes into the `*.po` language files.
- `make i18n-build` (or `./scripts/translate-build.sh`) will convert the `*.po` files to it's binary `*.mo` version and move it
